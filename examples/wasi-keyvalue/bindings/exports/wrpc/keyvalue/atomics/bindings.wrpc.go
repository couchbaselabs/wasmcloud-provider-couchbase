...
func ServeInterface(s wrpc.Server, h Handler) (stop func() error, err error) {
	...
	stop0, err := s.Serve("wrpc:keyvalue/atomics@0.2.0-draft", "increment", func(ctx context.Context, w wrpc.IndexWriteCloser, r wrpc.IndexReadCloser) {
		...
		if err != nil {
			slog.WarnContext(ctx, "failed to write result value", "i", 0, "wrpc:keyvalue/atomics@0.2.0-draft", "name", "increment", "err", err)
			return
		}
		if write0 != nil {
			writes[0] = write0
		...
		if err != nil {
			slog.WarnContext(ctx, "failed to write result", "wrpc:keyvalue/atomics@0.2.0-draft", "name", "increment", "err", err)
			return
		}
		if len(writes) > 0 {
			for index, write := range writes {
				w, err := w.Index(index)
				if err != nil {
					slog.ErrorContext(ctx, "failed to index writer", "index", index, "wrpc:keyvalue/atomics@0.2.0-draft", "name", "increment", "err", err)
					return
				}
				index := index
				write := write
				go func() {
					if err := write(w); err != nil {
						slog.WarnContext(ctx, "failed to write nested result value", "index", index, "wrpc:keyvalue/atomics@0.2.0-draft", "name", "increment", "err", err)
					}
				}()
			}
		}
